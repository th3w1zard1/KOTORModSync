    ...
    public static class DirectoryInfoExtensions
    {
            ...
            IEnumerator<T> enumerator)
        {
            while (true)
            {
                ...
                {
                    Logger.LogVerbose($"Permission denied while enumerating file/folder wildcards: {permEx.Message} Skipping...");
                    continue; // Skip files or directories with access issues
                }
                catch (IOException ioEx)
                {
                    Logger.LogError($"IO exception enumerating file/folder wildcards: {ioEx.Message} Skipping file/folder...");
                    continue; // Skip files or directories with IO issues
                }
                catch (Exception ex)
                {
                    Logger.LogError($"Unhandled exception enumerating file/folder wildcards: {ex.Message}. Attempting to skip file/folder item...");
                    continue;
                }
